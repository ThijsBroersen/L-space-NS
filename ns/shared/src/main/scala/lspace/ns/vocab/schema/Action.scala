...
lazy val result = lspace.ns.vocab.schema.result.property
lazy val startTime = lspace.ns.vocab.schema.startTime.property
lazy val target = lspace.ns.vocab.schema.target.property
}
override lazy val properties: List[LProperty] = List(actionStatus, agent, endTime, error, instrument, location, `object`, participant, result, startTime, target)
